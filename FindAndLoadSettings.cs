      ...
      solfile.SolutionFileLocation.ForEachValue_NotNull(lifetimeComponent, (lifetimeLocation, location) =>
      {
        ...
        for(FileSystemPath dir = location.Directory; !dir.IsNullOrEmpty(); dir = dir.Directory)
        {
          try
          {
            ...
            {
              string name = settingsfile.ConvertToRelativePath(location.Directory).FullPath.Replace("." + AutoLoadExtension, "");

              // Physical storage
              IProperty<FileSystemPath> livepath = new Property<FileSystemPath>(lifetimeLocation, "StoragePath", settingsfile);
              var storage = new XmlFileSettingsStorage(lifetimeLocation, name, livepath, SettingsStoreSerializationToXmlDiskFile.SavingEmptyContent.KeepFile, threading, filetracker, behavior);
              ...
              mount.Metadata.Set(UserFriendlySettingsLayers.Origin, string.Format("Automatically loaded from solution parent folder, “{0}”", settingsfile.ConvertToRelativePath(location.Directory).FullPath));
